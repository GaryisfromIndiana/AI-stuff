...
class KnowledgeMaintainer:
    ...
    def detect_lieutenant_gaps(self, min_cluster_size: int = 5) -> list[dict]:
        """Detect topic clusters that no existing lieutenant covers.

        When 5+ entities cluster around a topic outside any lieutenant's domain,
        this signals that a new specialist lieutenant should be spawned.

        Args:
            min_cluster_size: Minimum entities to trigger a spawn recommendation.
        ...
        try:
            ...
            existing_lts = lt_manager.list_lieutenants(status="active")
            covered_domains = {lt["domain"].lower() for lt in existing_lts}
            covered_names = {lt["name"].lower() for lt in existing_lts}
        except Exception:
            covered_domains = set()
            covered_names = set()

        # Group entities by type and look for uncovered clusters
        type_clusters: dict[str, list[str]] = {}
        for entity in entities:
            etype = entity.entity_type or "unknown"
            if etype not in type_clusters:
                type_clusters[etype] = []
            type_clusters[etype].append(entity.name)

        # Also cluster by keyword analysis of entity names/descriptions
        keyword_clusters: dict[str, list[str]] = {}
        topic_keywords = [
            "robotics", "quantum", "biotech", "crypto", "blockchain", "climate",
            "healthcare", "education", "finance", "gaming", "autonomous", "drone",
            "cybersecurity", "privacy", "chip", "semiconductor", "energy",
            "space", "satellite", "legal", "regulation", "governance",
        ]
        ...
        for entity in entities:
            ...
            for keyword in topic_keywords:
                if keyword in text:
                    ...
                    keyword_clusters[keyword].append(entity.name)

        # Find clusters that aren't covered by existing lieutenants
        recommendations = []

        for topic, entity_names in keyword_clusters.items():
            if len(entity_names) < min_cluster_size:
                continue

            # Check if any existing lieutenant covers this domain
            if topic in covered_domains:
                continue
            # Check name overlap
            if any(topic in name for name in covered_names):
                continue
            ...
            recommendations.append({
                ...
                "suggested_name": f"{topic.title()} Specialist",
                "suggested_domain": topic,
                "reason": f"{len(entity_names)} entities clustered around '{topic}' with no specialist lieutenant",
            })

        # Sort by entity count (biggest gaps first)
        recommendations.sort(key=lambda x: x["entity_count"], reverse=True)
        return recommendations[:5]  # Top 5 recommendations

    def auto_spawn_lieutenants(self, max_spawns: int = 2) -> list[dict]:
        """Automatically spawn new lieutenants for uncovered topic clusters.

        Args:
            max_spawns: Maximum lieutenants to spawn per run.

        Returns:
            List of spawned lieutenant info.
        """
        gaps = self.detect_lieutenant_gaps(min_cluster_size=5)
        if not gaps:
            logger.info("No lieutenant gaps detected — fleet is well-covered")
            return []
        ...
        try:
            ...
            for gap in gaps[:max_spawns]:
                try:
                    # Use LLM to generate a proper persona
                    persona = PersonaConfig(
                        name=gap["suggested_name"],
                        role=f"AI Research Specialist — {gap['topic'].title()}",
                        domain=gap["suggested_domain"],
                        expertise_areas=[gap["topic"], f"{gap['topic']} research", f"{gap['topic']} applications"],
                        system_prompt_template=(
                            f"You are {gap['suggested_name']}, a specialist in {gap['topic']}. "
                            f"You track the latest developments, research, and applications in {gap['topic']}. "
                            f"You provide expert analysis from the {gap['topic']} perspective."
                        ),
                    )

                    lt = lt_manager.create_lieutenant(
                        name=gap["suggested_name"],
                        persona=persona,
                        domain=gap["suggested_domain"],
                    )
                    ...
                    logger.info(
                        "Auto-spawned lieutenant: %s (domain=%s, %d entities triggered)",
                        gap["suggested_name"], gap["suggested_domain"], gap["entity_count"],
                    )

                except Exception as e:
                    logger.warning("Failed to spawn lieutenant for %s: %s", gap["topic"], e)
