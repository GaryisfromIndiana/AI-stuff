...
@empire_route
def api_auto_spawn(empire_id):
    """Auto-spawn lieutenants for uncovered topic clusters."""
    data = _get_json_or_400()
    from core.knowledge.maintenance import KnowledgeMaintainer
    spawned = KnowledgeMaintainer(empire_id).auto_spawn_lieutenants(
        max_spawns=min(data.get("max_spawns", 2), 5),
    )
...
@rate_limit(requests_per_minute=5, requests_per_hour=50)
@empire_route
def api_generate_content(empire_id):
    ...
    content = ContentGenerator(empire_id).generate(
        topic=topic,
...
@empire_route
def api_content_from_directive(empire_id, directive_id: str):
    """Generate formatted content from a completed directive."""
    data = _get_json_or_400() or {}
    from core.content.generator import ContentGenerator
    content = ContentGenerator(empire_id).generate_from_directive(
        directive_id=directive_id,
        template_name=data.get("template", "research_briefing"),
    )
    return {
...
@empire_route
def api_status_report(empire_id):
    """Generate an Empire status report."""
    from core.content.generator import ContentGenerator
    content = ContentGenerator(empire_id).generate_status_report()
    return {
        "title": content.title,
        "sections": [{"title": s.title, "content": s.content, "words": s.word_count} for s in content.sections],
        "total_words": content.total_words,
...
@empire_route
def api_render_report(empire_id, directive_id: str):
    """Render a directive report as HTML page."""
    from core.content.generator import ContentGenerator
    content = ContentGenerator(empire_id).generate_from_directive(
        directive_id, request.args.get("template", "research_briefing"),
    )
    return Response(content.html, mimetype="text/html")


@api_bp.route("/content/weekly-digest", methods=["POST"])
@empire_route
def api_weekly_digest(empire_id):
    """Generate a weekly AI digest from the last 7 days."""
    from core.content.generator import ContentGenerator
    content = ContentGenerator(empire_id).generate_weekly_digest()
    return {
        "title": content.title,
...
@empire_route
def api_stored_reports(empire_id):
    """Get previously generated reports."""
    from core.content.generator import ContentGenerator
    return ContentGenerator(empire_id).get_stored_reports(limit=_clamp_limit(10))


@api_bp.route("/content/render-topic", methods=["POST"])
@empire_route
def api_render_topic(empire_id):
    ...
    from core.content.generator import ContentGenerator
    content = ContentGenerator(empire_id).generate(
        topic=topic, template_name=data.get("template", "research_briefing"),
    )
    return Response(content.html, mimetype="text/html")
...
@rate_limit(requests_per_minute=5)
@empire_route
def api_enrichment_run(empire_id):
    """Trigger a shallow enrichment cycle."""
    data = request.get_json(silent=True) or {}
    from core.research.enrichment import ShallowEnricher
    result = ShallowEnricher(empire_id).run_enrichment_cycle(
...
@empire_route
def api_cross_synthesis_overlaps(empire_id):
    ...
    overlaps = CrossLieutenantSynthesizer(empire_id).detect_overlaps()
    return [
        {
            "topic": o.topic,
            "domains": o.domains,
...
@rate_limit(requests_per_minute=2, requests_per_hour=10)
@empire_route
def api_cross_synthesis_run(empire_id):
    """Run cross-lieutenant synthesis cycle."""
    data = request.get_json(silent=True) or {}
    from core.research.cross_synthesis import CrossLieutenantSynthesizer
    result = CrossLieutenantSynthesizer(empire_id).run_synthesis_cycle(
